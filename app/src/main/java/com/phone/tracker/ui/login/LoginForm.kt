...
@Preview
@Composable
fun LoginForm(state: LoginResposeModel = LoginResposeModel(), login: (Credentials) -> Unit = {}) {
    val pref = rememberPrefManager()
    val context = LocalContext.current // Get the current context
    LaunchedEffect(state) {
        if (state.status == 1) {
            pref.userIdSave(state.login.first().userId)
            if (!pref.userIdGet().isNullOrEmpty()) {
                // Start MainActivity
...
data class Credentials(
    var username: String = "5111111114",
...
fun PasswordField(
    ...
    val trailingIcon = @Composable {
        IconButton(onClick = { isPasswordVisible = !isPasswordVisible }) {
            Icon(
                if (isPasswordVisible) Icons.Default.Clear else Icons.Default.Lock,
                contentDescription = "",
                tint = MaterialTheme.colorScheme.primary
            )
        }
